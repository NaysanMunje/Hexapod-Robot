...
<body>
  ...
  <script type="module">
    ...
    const HIP_THICK = 0.010;
    const THIGH_THICK = 0.030;
    ...
    document.querySelectorAll('.tab').forEach((btn) => {
      btn.onclick = async () => {
        ...
        if (mode === 'twist') freezeEl.checked = false;
        if (isStretchTab()) {
          playing = true;
    ...
    Object.values(ui).forEach((el) => el.addEventListener('input', markDirty));
    ...
    async function loadFromRobot() {
      try {
        const p = await (await fetch('/api/walk/get')).json();
        applyParamsToUI(p);
        robotOn = !!p.enabled;
        robotBtn.textContent = robotOn ? 'Stop robot' : 'Start robot';
        robotBtn.className = robotOn ? 'stop' : 'go';
        ...
        else if (p.enabled && ({ rotate:1, pitch:1, crouch:1, twist:1, roll:1, nod:1, swirl:1 })[sm]) mode = sm;
        syncTabs();
        setTabPanels();
        setRobotText('Robot: ' + (robotOn ? (sm !== 'off' ? 'STRETCH ' + sm : 'WALKING') : 'STOPPED') +
    ...
    function update(dt) {
      ...
      for (const leg of legs) {
        ...
        shinSeg[h.name] = kneeFootBody(h, coxaShow, j.femur, fx, fy, fz);
      ...
      if (sk === 'linear') {
        ...
        motion = lean.leanX >= 0 ? 'pitch forward' : 'pitch back';
      } else if (sk === 'crouch') {
        motion = lean.dHeight >= 0 ? 'crouch down' : 'crouch up';
      } else if (sk === 'twist') {
        motion = `twist ${(lean.yaw / DEG).toFixed(0)}°`;
      } else if (sk === 'roll') {
        motion = lean.roll >= 0 ? 'roll left-up' : 'roll right-up';
      } else if (sk === 'nod') {
        motion = lean.pitch >= 0 ? 'nod nose-down' : 'nod tail-down';
      } else if (sk === 'swirl') {
        motion = `swirl ${((stretchPhase * 360) % 360).toFixed(0)}°`;
      } else if (sk === 'rotate') {
