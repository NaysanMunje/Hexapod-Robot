...
<body>
  <div id="ui">
    ...
    <div id="hint">Sliders update the 3D preview only.<br/>Deploy sends params; Start runs the robot.<br/>Needs internet for Three.js CDN.<br/>Drag orbit · scroll zoom</div>
  </div>
  ...
  <script type="module">
    ...
    shape.closePath();
    const plateGeom = new THREE.ShapeGeometry(shape);
    plateGeom.rotateX(-Math.PI / 2);
    robot.add(new THREE.Mesh(
      plateGeom,
      new THREE.MeshStandardMaterial({ color: 0x4a7dc0, transparent: true, opacity: 0.4, side: THREE.DoubleSide })
    ));
    ...
    function stick(length, radius, color) {
    ...
    function shinPlate(length, color) {
      const g = new THREE.BoxGeometry(length, 0.008, SHIN_WIDTH);
      g.translate(length / 2, 0, 0);
      return new THREE.Mesh(g, new THREE.MeshStandardMaterial({
        color, metalness: 0.05, roughness: 0.7, transparent: true, opacity: 0.85,
      }));
    }
    ...
    function makeLeg() {
      ...
      root.add(coxaYaw);
      coxaYaw.add(stick(L_COXA, 0.0035, 0xcc4444));
      coxaYaw.add(ball(0.005, 0xffffff));

      const femurPivot = new THREE.Group();
      femurPivot.position.x = L_COXA;
      coxaYaw.add(femurPivot);
      femurPivot.add(ball(0.0045, 0x88ccff));
      femurPivot.add(stick(L_FEMUR, 0.0035, 0x3399ee));

      const kneePivot = new THREE.Group();
      kneePivot.position.x = L_FEMUR;
      femurPivot.add(kneePivot);
      kneePivot.add(ball(0.0045, 0x66ee99));
      kneePivot.add(shinPlate(L_S1, 0x33cc66));

      const bend1 = new THREE.Group();
      bend1.position.x = L_S1;
      bend1.rotation.z = BEND1;
      kneePivot.add(bend1);
      bend1.add(shinPlate(L_S2, 0x2aa855));

      const bend2 = new THREE.Group();
      bend2.position.x = L_S2;
      bend2.rotation.z = BEND2;
      bend1.add(bend2);
      bend2.add(shinPlate(L_S3, 0x1f8844));
      const foot = ball(0.005, 0xf0d020);
    ...
    const legs = hips.map((h) => {
      ...
      spr.position.copy(toThree(h.x * 1000, h.y * 1000, 22));
      robot.add(spr);
      return leg;
    });
